...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
        ...
        x = tf.placeholder(
        tf.float32, [
            mnist.validation.images.shape[0],                   # 第一维表示一个batch中样例的个数
        ...
        validate_feed = {x: np.reshape(mnist.validation.images, (mnist.validation.images.shape[0],
                                          mnist_inference.IMAGE_SIZE,
                                          mnist_inference.IMAGE_SIZE,
        ...
        # 因为测试时不关注正则化损失的值, 所以这里用于计算正则化损失的函数
        # 被设置为None
        ...
        # 通过变量重命名的方式来加载模型, 这样在前向传播的过程中就不需要调用
        # 求滑动平均的函数来获取平均值了。这样就可以完全公用mnist_inference.py中
        # 定义的前向传播过程
        ...
        # 每隔EVAL_INTERVAL_SECS秒调用一次计算正确率的过程以检测训练过程中正确率
        # 的变化
